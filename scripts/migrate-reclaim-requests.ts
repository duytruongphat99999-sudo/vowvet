/**
 * RECLAIM-REQUESTS migration — bảng MỚI `reclaim_requests` (yêu cầu lấy lại bé, Hướng B).
 * Fields: pet_id(num), pet_name, passport_code, requester_id(num), requester_name,
 *         handover_id(num), status(text: pending|resolved|expired), notified(bool), created_at(text).
 *
 * ANTI-CLOBBER: nếu bảng reclaim_requests ĐÃ tồn tại → DỪNG, không tạo đè.
 * Run trên HOST: bun run scripts/migrate-reclaim-requests.ts
 */
...
if (tablesBefore.find((t) => t.name === "reclaim_requests")) {
  const t = tablesBefore.find((t) => t.name === "reclaim_requests")!;
  console.log(`⛔ Bảng reclaim_requests ĐÃ tồn tại (id=${t.id}) — DỪNG, không tạo đè. Báo Duy.`);
  process.exit(0);
}

// ── TẠO BẢNG + FIELD ──────────────────────────────────────────
console.log(`🔄 Tạo bảng reclaim_requests…`);
const table = await api<TableDef>(`/database/tables/database/${DATABASE_ID}/`, {
  method: "POST", body: JSON.stringify({ name: "reclaim_requests" }),
});
console.log(`  + table reclaim_requests id=${table.id}`);

const FIELDS = [
  { name: "pet_id", type: "number", number_decimal_places: 0, number_negative: false },
  { name: "pet_name", type: "text" },
  { name: "passport_code", type: "text" },
  { name: "requester_id", type: "number", number_decimal_places: 0, number_negative: false },
  { name: "requester_name", type: "text" },
  { name: "handover_id", type: "number", number_decimal_places: 0, number_negative: false },
  { name: "status", type: "text" }, // pending | resolved | expired
...
config.tables.reclaim_requests = { id: table.id, fields: {} };
for (const f of fresh) if (f.name) config.tables.reclaim_requests.fields[f.name] = f.id;
...
console.log(`reclaim_requests id=${table.id}, fields: ${Object.keys(config.tables.reclaim_requests.fields).join(", ")}`);
