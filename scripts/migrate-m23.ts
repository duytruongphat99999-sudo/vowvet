/**
 * M23 — Pain (Glasgow CMPS-SF) + Mobility assessments tables.
 * Idempotent.
 */
...
const MOBILITY_FIELDS = [
  { name: "pet_id", type: "link_row", link_row_table_id: PETS_TABLE },
  { name: "assessed_at", type: "text" },
  { name: "raw_score", type: "number", number_decimal_places: 0 },
  { name: "pct_score", type: "number", number_decimal_places: 0 },
  {
    name: "mobility_level",
    type: "single_select",
    select_options: [
...
const painTable = await ensureTable("pain_assessments", PAIN_FIELDS);
const mobilityTable = await ensureTable("mobility_assessments", MOBILITY_FIELDS);

const config: any = JSON.parse(JSON.stringify(existingConfig));
for (const [name, t] of [["pain_assessments", painTable], ["mobility_assessments", mobilityTable]] as const) {
