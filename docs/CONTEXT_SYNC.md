...
## 📍 ĐUÔI PHIÊN 2026-06-08 (tối, sau migration) — VERIFY UX + việc next

### ✅ XÁC NHẬN CHẠY ĐÚNG (qua ảnh user, post-cutover prod build)
- Camera scan SỐNG: /pets/<id> tab Dinh dưỡng → "Scan nhãn thức ăn" có input capture=environment + nút chụp. Label guardrail đúng ("AI của VowVet phân tích — tham khảo, không thay bác sĩ"). KHÔNG phải input tay.
- Login Google init OK: /api/v1/auth/google → 302 accounts.google.com, redirect_uri trỏ live đúng. (Callback+set-session: user tự xác nhận khi login thật — chưa thấy báo vỡ.)
- Bài học phiên này: PHẦN LỚN "lỗi" user thấy = SW v298 + CF cache giữ bản cũ, KHÔNG phải bug. Tab thường + PWA đã cài đều bị che. Test ĐÚNG = incognito hoặc curl từ container. (Đã verify build qua docker exec grep dist, cache-proof.)
- 404 "login lỗi" thực ra = test nhầm cổng 127.0.0.1:4322/api (container prod KHÔNG có /api proxy — chỉ NPM route /api trên live).

### 🟡 VIỆC NEXT — FAB "Quét" bật camera 1 chạm (chưa làm, cần build tỉnh táo)
**Mục tiêu**: bấm FAB Quét → mở camera NGAY (bỏ nhịp land trang giữa rồi mới tap chụp).
**RÀNG BUỘC (đã recon, quan trọng)**:
- MPA thuần, 0 View Transitions/ClientRouter → mỗi nav = FULL RELOAD → user-gesture KHÔNG carry qua trang → input.click() trong x-init sau nav bị iOS CHẶN CÂM. ⇒ auto-open-after-nav BẤT KHẢ THI.
- Camera mở 1-chạm CHỈ được nếu .click() nổ trong CÙNG gesture, không reload.
**HƯỚNG CHỐT = A (same-page) + B (cross-page)**:
- A: khi ĐANG ở /pets/<id> → FAB KHÔNG href-nav, mà onclick gọi đồng bộ document.getElementById('food-scan-input-'+petId).click() (+ scroll #food-scan). Gesture nguyên → camera mở.
- B: khác trang → vẫn <a href> /pets/<id>?scan=1 như hiện tại (land + tap thêm 1 lần). Đúng chuẩn trình duyệt.
- C (thêm ClientRouter SPA) = LOẠI (phức tạp, gesture-qua-nav vẫn không tin được).
**CHƯA ĐỦ DATA — recon thêm trước khi build**:
- BottomNav.astro (global) lấy petId active kiểu gì? detect "đang ở trang /pets/<id>" kiểu gì? (window.location.pathname?)
- Input chỉ render khi ở trang pet + tab nutrition đã mở → khi cùng trang nhưng tab khác, .click() có thấy input không? (cần ensure tab nutrition active trước khi click, đồng bộ trong gesture.)
- inputId = `food-scan-input-${petId}` (đã biết). foodScanWidget THIẾU method trigger (không openPicker/$refs) → sẽ thêm.
**ĐỘ RỦI RO**: A đụng BottomNav.astro = file GLOBAL (mọi trang) → sửa sai = lỗi nav toàn app. Cần RECON kỹ + verify nhiều trang. Build phiên riêng, tỉnh táo.

### 🟢 LẺ (gộp 1 phiên dọn)
- BottomNav.astro typo nghi ngờ: href="\vaccines" (:54) + "\*" (:203) — dấu \ ngược, lẽ ra /vaccines + /*; \* có thể phá comment CSS. RECON-guard trước khi sửa (chưa rõ hại thật không).
- Cruft sw.js.v184.bak bị serve công khai (web/public/) — git rm.
- Bind-mount src/public/astro.config thừa trên prod (footgun "sửa host không vào dist") — bỏ khỏi compose lúc dọn.

### ⏳ NỢ CŨ vẫn treo (từ block trên)
- CF purge: đẩy nav/scan ra phone NHANH (không gấp — tự hết hạn ~10h; .env KHÔNG có CF token nên purge = tay dashboard Purge Everything). User đang test incognito nên không chặn.
- loop route-302 (Layout controllerchange→reload không guard) — fix TRƯỚC lần bump SW VERSION kế.

---

## 📍 PHIÊN 2026-06-08 (khuya) — FAB 1-CHẠM CAMERA: SHIP + VERIFIED iPhone

### ✅ XONG — verified trên iPhone thật (incognito), 3/3 mốc
FAB "Quét" giờ ĐI TẮT: bấm → camera mở thẳng → chụp → sheet chọn pet → OCR → nhảy /pets/<id>?scan=1 → màn scan cũ hiện kết quả + scroll. Cắt bỏ "profile→lối sống→dinh dưỡng". User test iPhone: camera mở ✓ · sheet ✓ · OCR ra đúng pet ✓.

### Commit (local-only)
- ce2e3f5 — feat(scan): FAB→camera→pet picker→OCR→handoff sessionStorage→foodScanWidget.init().
- 2154140 — style(nav): reset native chrome <button> FAB (border/bg/font, match button.vv-bn-item).

### Kiến trúc đã build (để phiên sau hiểu, đừng phá)
- Input camera ẩn ĐẶT TRONG BottomNav.astro (global, cùng gesture FAB → né iOS gesture-loss, mở mọi trang). x-ref=fabScanInput, capture=environment.
- Sheet chọn pet: lấy d.pets từ /api/v1/auth/me (đủ field id/name/species/photo_url). 1 pet → bỏ sheet scan thẳng. Nhiều pet → sheet (style INLINE viết tay, CHƯA polish — xấu nhưng chạy).
- OCR upload Ở BottomNav (File mất khi reload nên KHÔNG nav trước): POST /api/v1/pets/<chosenId>/food/scan → result.scan_url (R2, sống qua nav).
- Handoff: sessionStorage["vowvet:scan:<id>"] = result JSON → nav → foodScanWidget.init() (MỚI THÊM ở pets/[id].astro) đọc lên render + removeItem + scroll. preview = scan_url (không cần blob).
- KHÔNG đụng: nutrition*.ts, API, schema. Label vẫn "AI của VowVet".

### ⚠️ Đính chính từ block trước
- Typo "\vaccines"/"\*" trong BottomNav = ẢO (artifact escape của Grep tool, file ĐÚNG /vaccines + /*). GẠCH khỏi việc dọn.

### 🟡 Polish/nợ FAB flow (chưa gấp)
- Sheet chọn pet style inline thô → cần vuốt lại (tách component hoặc style đẹp). Tham chiếu mẫu: map.astro:150 bottom-sheet, care-plan/ConsentModal.astro a11y.
- Test thêm: nhiều pet chọn pet thứ 2/3 đúng không; lỗi mạng giữa OCR; ảnh >10MB; pet 0 (user chưa có pet) bấm FAB.

### ⏳ NỢ CŨ vẫn treo
- **CF purge** (ƯU TIÊN giờ): FAB flow + nav/scan CHƯA ra phone thường/PWA (SW v298 + CF cache giữ bản cũ). .env KHÔNG có CF token → purge TAY: dashboard monminpet.com → Caching → Purge Everything. An toàn (push đúng, SW không loop). Sau purge: phone hard-refresh / gỡ-cài lại PWA.
- loop route-302 (Layout controllerchange→reload không guard) — fix TRƯỚC lần bump SW VERSION kế.
- Cruft sw.js.v184.bak serve công khai (git rm) · bind-mount thừa trên prod.
- Phao .bak (compose/astro.config/BottomNav/pets[id]/web.Dockerfile dev) — prod ổn rồi, dọn được sau ~1 ngày.
