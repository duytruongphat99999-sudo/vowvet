...
> 🆕 **cập nhật 2026-06-06 (MỚI NHẤT):** **Design-system food-brands + Bottom-nav app-style.** (1) **Áp 1 hệ thiết kế /food-brands**: emoji→**FeatureIcon** (cat/dog/sun/**moon mới**/sparkles/alert-triangle; bcsAvatar/breedSticker + env 🧬🏢💧 GIỮ — cụm sau) · bỏ **emerald**→gold/ink (§9; còn sót **bcsAvatar** = CẤM ĐỤNG) · hierarchy (giá/gram **bold ink** · thanh tương thích **xám** · mismatch **dịu amber** "Lưu ý", bỏ serif đỏ/🚨) · MonMin name **to/đậm gold-deep** + badge sparkles · affordance gập (`hover:bg` + chữ **"Xem/Thu gọn"**) · filter read-only gập **"(chỉ xem)"** khi locked · header `<img>`→**`<Logo>`**. (v288 `c4b23ef`) (2) **Bottom-nav app-style** (`BottomNav.astro` MỚI, gắn `Layout` sau `<slot/>` → phủ mọi trang gated): **5 ô** (Trang chủ·Dinh dưỡng·**Quét giữa nổi tròn gold**·Theo dõi·Tôi) · petId **fetch /auth/me** · active **SSR** · **safe-area iOS** (`viewport-fit=cover`+`env(safe-area-inset-bottom)`+`.has-bottom-nav`) · **GATE** (login AND không landing/login/onboarding/offline + error qua **`Astro.routePattern` /404,/500** + public `/p/ /memorial/ /personality-card/`). Quét = **deep-link** `#smart-scanner` (camera = Mạch 2). (v289 `3fe0eb2`). **SW = `vowvet-v289-app-bottom-nav`** · HEAD `3fe0eb2` · **+19 commit local CHƯA push**. Xem **(tiếp) DESIGN + BOTTOM-NAV** trong SESSION 2026-06-06.
> 🆕 **cập nhật 2026-06-06 (sáng):** **Audit DER + Lớp 2 + UI humanize + mobile-first + fix bug species** — engine A về **AAHA/WSAVA** (sedentary default, bỏ phạt senior/geri, +`ageingNote`; RER giữ) · **1 NGUỒN DER** · pet page **Mức nền/Hôm nay** · data-fix **min activity→sedentary** · **Lớp 2**: bịt rò ghi-cân + nhắc cân + đề xuất khẩu phần + apply guard + whitelist `target_weight_kg` · **#2: gram_nền GIỮ BCS, bỏ weather** · **Dọn UI tiếng người** (2 trang) · **Lát 1+2 mobile-first** (khẩu phần lên đầu + section gập + auto-expand theo cờ BCS/bệnh/dị ứng/catalog) · **🐛 FIX bug species nhãn-VN→enum** ở food-brands `selectPet` (mèo giờ thấy **6 brand**, trước = 0 do `"Mèo" !== "cat"`). **SW = `vowvet-v287-species-enum-normalize`** · HEAD `0ccd61e` · **+16 commit local CHƯA push**. Xem **SESSION 2026-06-06** ngay dưới.
...
### 📊 STATE
- HEAD `3fe0eb2` · **SW `vowvet-v289-app-bottom-nav`** · **+19 commit local CHƯA push** (a8e61b7→3fe0eb2; + commit save này). *(Cập nhật mốc DESIGN+NAV; STATE cũ: `0ccd61e`/v287/+16.)*
...
14. `8464940` **Mobile-first lát 1** (food-brands): sắp xếp lại thứ tự — **khẩu phần (DER/gram) lên đầu** + deep section **collapsible** (gập/mở). CHỈ layout/thứ tự — số không đổi. (v285)
15. `f47902a` **Mobile-first lát 2** (food-brands): **auto-expand theo ngữ cảnh** — `applyContextExpand()` chỉ MỞ THÊM (không tự đóng) theo cờ: BCS≠ideal → Goal+Profile · activeConditions → Goal · allergens → Filter · `speciesHasCatalog && filtered=0` → More. Reduced-motion safe. (v286)
16. `0ccd61e` **🐛 FIX bug species nhãn-VN→enum** (food-brands `selectPet` :1452, 1 chỗ): `/auth/me`→`toApiPet:112` trả species **VN** (`'cat'`→`'Mèo'`, contract cố ý), nhưng filter/scoring/speciesHasCatalog so **enum** `'cat'/'dog'/'both'` → `"Mèo" !== "cat"` lọc HẾT 6 brand mèo (grid rỗng). Fix: normalize VN→enum phòng thủ 2 chiều (`"Mèo"||"cat"→"cat"`…) như backend `matchBrand:319` + `pets/[id].astro:3110 medSpecies`. **KHÔNG đụng** api contract/schema/DER. Verify DOM authed (min pet 12 mèo): profile.species `"cat"` (rawSpecies API="Mèo"), filtered **6** (trước 0), DOM 6 card mèo + 0 chó, manual Chó→8/Mèo→6, **DER y nguyên 234/45/83**, console sạch. (v287)
...
2. **care-plan-engine dùng engine LIVE** thay field `daily_calorie_target` đông cứng (đụng `api/`, cụm riêng).
3. **Bundle manual food-brands**: `derClient` BCS-only → import/gọi engine thật (reactive client, không cần lưu).
4. **Nước 50 vs 55**: engine `weight×50` (pet page) vs `care-plan.astro:519` `weight×55 min50` vs field `daily_water_ml` lưu → thống nhất 1 nguồn.
...
> ✅ **Lát 1+2 mobile-first ĐÃ ĐÓNG** (khẩu phần lên đầu + section gập + auto-expand theo cờ) + **🐛 bug species ĐÃ XONG** (mèo thấy 6 brand) — tính năng "thức ăn hợp với bé" SỐNG LẠI cho mèo (trước chết do mismatch nhãn-VN vs enum).

---

## 🆕 (tiếp 2026-06-06) — DESIGN-SYSTEM food-brands + BOTTOM-NAV app-style

> Arc: recon design/IA food-brands → áp 1 hệ thiết kế (FeatureIcon/palette/hierarchy/affordance) → recon app-shell → dựng bottom-nav 5 ô gắn Layout (gate + safe-area). *(APPEND.)*

### ✅ ĐÃ XONG (3 commit, sau `0ccd61e`)
- `a716c38` docs(context): save mobile-first + species fix.
- `c4b23ef` **ui(nutrition): unify design system /food-brands (v288)** — `food-brands.astro` + `FeatureIcon.astro`. emoji→FeatureIcon: 🐱🐶→cat/dog (card x-show toggle + nav/widget avatar GIỮ breedSticker + toggle Loài) · 🚨⚠️→alert-triangle (mismatch + matrix tier1/2 + budget) · ✨→sparkles (MonMin badge + filter + combo + budget + poop-tap) · ☀️→sun · 🌙→**moon (icon MỚI thêm FeatureIcon)**. emerald→gold/ink (§9): verified badge ×2, BCS ideal label+slider, goal "đang ổn", raw kcal tile, conditionNotes, fedToday, scanner result, guest CTA, poop tap. Hierarchy: giá/ngày+giá/kg→`font-bold text-mmp-ink`; thanh tương thích→1 màu `bg-mmp-ink/20`+%`/45` (mọi card ~75% → màu vô nghĩa); mismatch banner→`text-amber-700`+alert-triangle+"Lưu ý:" (bỏ `font-display rose-700`+🚨), border card mismatch rose→`amber-200/50`. MonMin name `:class` = `text-xl font-bold text-[#B48608]` (thường giữ `italic text-lg`). Affordance: 6 header gập +`hover:bg-mmp-cream/40 motion-safe:transition` + wrap chevron `<span>` chữ "Xem/Thu gọn"; openBrands/openMore thêm nền+viền. Filter: `openAllergyView` flag → khi `isProfileLocked` 4 ma trận bọc accordion **"Hồ sơ dị ứng & môi trường (chỉ xem)"** mặc định đóng (`x-show="!isProfileLocked || openAllergyView"`). Header: khối `<img logo>` thủ công → `<Logo variant="default" href="/">`. **KHÔNG đụng số/scoring/mismatch-logic** — DER 234/45/83 + grams card 246/67/62/62/59/65 nguyên.
- `3fe0eb2` **feat(shell): app-style bottom navigation (v289)** — `BottomNav.astro` MỚI + `Layout.astro` + `global.css` + `FeatureIcon.astro` (+scan/+chart). Xem mục KIẾN TRÚC dưới.

### 🧭 KIẾN TRÚC BOTTOM-NAV (đọc trước khi đụng shell)
- **`components/BottomNav.astro`** = `<nav class="vv-bottomnav lg:hidden print:hidden">` + scoped `<style>`. 5 `<a>`: Trang chủ(/dashboard,home) · Dinh dưỡng(/food-brands,bowl) · **Quét**(`/food-brands#smart-scanner`, nút tròn gold `margin-top:-26px` nổi, icon `scan`) · Theo dõi(`:href` /pets/{petId}/care-plan, chart) · Tôi(`:href` /pets/{petId}, user). Tông cream/gold/ink, `is-active`=gold-deep `#b48608`.
- **petId**: `<script is:inline> bottomNav(){ petId:null, loadPet(){ fetch('/api/v1/auth/me',{credentials:'include'}) → pets[0].id } }`, `x-init="loadPet()"`. Chưa có pet → `:href` fallback `/dashboard` (static `href` cũng /dashboard).
- **Active SSR**: `Layout` truyền `path={Astro.url.pathname}` → BottomNav tính `isHome/isNutrition/isCare(/care-plan)/isMe(/^\/pets\/[^/]+\/?$/)` (no flash).
- **Gate (Layout)**: `showBottomNav = !!Astro.locals.user && routePattern∉['/404','/500'] && navPath∉['/','/login','/onboarding','/offline'] && !startsWith('/login','/p/','/memorial/','/personality-card/')`. ⚠️ **error page bắt qua `Astro.routePattern`** ('/404','/500') — `Astro.response.status` đọc **200** trong Layout (adapter set 404 SAU render) → status-gate VÔ DỤNG; routePattern là signal đúng (dashboard='/dashboard', unknown-path='/404').
- **Layout**: render `{showBottomNav && <BottomNav path={navPath}/>}` sau `<slot/>` + `<body class:list={[showBottomNav && "has-bottom-nav"]}>` + viewport meta += `viewport-fit=cover`.
- **global.css**: `.has-bottom-nav{padding-bottom:calc(4rem + env(safe-area-inset-bottom))}` · `@media(min-width:1024px){.has-bottom-nav{padding-bottom:0}}` · `@media print{.has-bottom-nav{padding-bottom:0!important}; .vv-bottomnav{display:none!important}}`.

### ⚠️ GOTCHA phiên này
- **Scoped style đè Tailwind**: Astro scoped `.vv-bottomnav{display:flex}` có `[data-astro-*]` (specificity 0,2,0) **THẮNG** Tailwind `.lg:hidden`/`.print:hidden` (0,1,0) → desktop KHÔNG ẩn. Fix: ẩn **trong scoped** `@media(min-width:1024px){.vv-bottomnav{display:none}}` + `@media print{...}` (cùng specificity, sau → thắng). Bài học: muốn responsive/print override 1 element có scoped style → viết media query TRONG scope.
- **Headless preview KHÔNG chạy `requestAnimationFrame`** (`rafFired:false`) → Alpine `x-transition:enter` (motion-safe, từ lát-1) **kẹt** ở `opacity-0/display:none` khi mở section → collapse "không mở" TRONG PREVIEW. **KHÔNG phải bug** (state đã-commit v287 kẹt y hệt; real browser chạy RAF → mở OK). Verify open-state phải bypass transition thủ công.
- **/login authed redirect /dashboard** → không verify gate /login trực tiếp được khi đã login (moot — user authed không thấy login).

### 🎯 VIỆC ĐANG DỞ (Bồ giao tiếp — cụm app-shell)
1. **Nút Quét auto-MỞ scanner**: hiện chỉ **scroll** tới `#smart-scanner` (section collapsed) — chưa auto `openScanner=true` vì CẤM ĐỤNG food-brands logic. → gộp **Mạch 2 (camera thật)**: thêm hash-handler food-brands (`location.hash==='#smart-scanner'`→open) HOẶC route `/scan` + getUserMedia/input-capture + API OCR.
2. **Header chuẩn hóa `AppHeader.astro`** (thay nav rời food-brands/pet page/các trang) — CHƯA làm (đại phẫu nhiều trang, deferred).
3. **/scan route + camera thật** — chưa tạo (Mạch 2). App hiện KHÔNG có getUserMedia; "camera" = `<input type=file>` (bills/bcs/photos → API Gemini vision).
4. **emoji còn sót /food-brands** (cụm sau, ngoài remove-list lần này): **bcsAvatar** 😿😸😾🎈🦴 + warn ✨ (CẤM ĐỤNG) · **env tier** 🧬🏢💧🌫️🦠 · breedSticker 29 emoji · `:946` ⚠️ ingredient-guard (x-text, ẩn với min) · ✅ scanner-result. Kèm **emerald bcsAvatar** (`:1761` bg/ring + warnColor) chưa đổi (CẤM ĐỤNG).

### 💾 BACKUP
- Bundle off-machine gần nhất `vowvet-20260606-1643.bundle` = **TRƯỚC** `e01c180` → giờ **TRỄ ~9 commit** (thiếu `e01c180` · `89bfd82` · `8464940` · `f47902a` · `0ccd61e` · `a716c38` · `c4b23ef` · `3fe0eb2` · save này). ⚠️ **CẦN bundle mới** qua `3fe0eb2` (+ commit CONTEXT_SYNC): `git bundle create C:\docker\backups\vowvet-<YYYYMMDD-HHMM>.bundle --all` → verify + clone-test → **kéo cloud thủ công** (repo CHƯA có remote).
